...
foreign lib {
	...
	/// (0) on failure.
	///
	time_to_timet :: proc (cef_time: ^time, time: ^c.time_t) -> c.int ---
	time_from_timet :: proc (time: c.time_t, cef_time: ^time) -> c.int ---
	...
	/// false (0) on failure.
	///
	time_to_base_time :: proc (from: ^time, to: ^base_time) -> c.int ---

	///
	/// Converts base_time to time. Returns true (1) on success and false (0) on
	/// failure.
	///
	time_from_base_time :: proc (from: base_time, to: ^time) -> c.int ---
}
