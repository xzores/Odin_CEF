...
// Print_settings is defined in Print_settings_capi.odin
...
Print_handler :: struct {
    ...
    /// print button).
    on_print_start: proc "c" (self: ^Print_handler, browser: ^Browser),

    /// Synchronize |settings| with client state. If |get_defaults| is true (1)
    /// then populate |settings| with the default print settings. Do not keep a
    /// reference to |settings| outside of this callback.
    on_print_settings: proc "c" (self: ^Print_handler, browser: ^Browser, settings: ^Print_settings, get_defaults: b32),

    /// Show the print dialog. Execute |callback| once the dialog is dismissed.
    /// Return true (1) if the dialog will be displayed or false (0) to cancel the
    /// printing immediately.
    on_print_dialog: proc "c" (self: ^Print_handler, browser: ^Browser, has_selection: b32, callback: ^print_dialog_callback) -> b32,

    /// Send the print job to the printer. Execute |callback| once the job is
    /// completed. Return true (1) if the job will proceed or false (0) to cancel
    /// the job immediately.
    on_print_job: proc "c" (self: ^Print_handler, browser: ^Browser, document_name: ^cef_string, pdf_file_path: ^cef_string, callback: ^print_job_callback) -> b32,

    /// Reset client state related to printing.
    on_print_reset: proc "c" (self: ^Print_handler, browser: ^Browser),

    /// Return the PDF paper size in device units. Used in combination with
    /// browser_host::print_to_pdf().
    get_pdf_paper_size: proc "c" (self: ^Print_handler, browser: ^Browser, device_units_per_inch: c.int) -> cef_size,
}
