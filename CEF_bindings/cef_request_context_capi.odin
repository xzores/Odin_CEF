...
Resolve_callback :: struct {
	...
	on_resolve_completed: proc "c" (self: ^Resolve_callback, result: cef_errorcode, resolved_ips: string_list),
}
...
Request_context :: struct {
	...
	is_same: proc "c" (self: ^Request_context, other: ^Request_context) -> c.int,

	// Returns 1 if this object shares the same storage as |other|.
	is_sharing_with: proc "c" (self: ^Request_context, other: ^Request_context) -> c.int,

	// Returns 1 if this is the global context (used by default when context arg is nil).
	is_global: proc "c" (self: ^Request_context) -> c.int,

	// Returns the handler for this context, if any.
	get_handler: proc "c" (self: ^Request_context) -> ^Request_context_handler,

	// Cache path for this object, or nil for incognito/in-memory cache. (free with cef_string_userfree_free)
	get_cache_path: proc "c" (self: ^Request_context) -> cef_string_userfree,

	// Cookie manager for this object. If |callback| non-nil, executed on UI thread after storage init.
	get_cookie_manager: proc "c" (self: ^Request_context, callback: ^Completion_callback) -> ^Cookie_manager,
	...
	register_scheme_handler_factory: proc "c" (
	...
	clear_scheme_handler_factories: proc "c" (self: ^Request_context) -> c.int,
	...
	clear_certificate_exceptions: proc "c" (self: ^Request_context, callback: ^Completion_callback),

	// Clear all HTTP auth credentials added via GetAuthCredentials. Optional |callback| on UI thread after completion.
	clear_http_auth_credentials: proc "c" (self: ^Request_context, callback: ^Completion_callback),

	// Clear all active/idle connections (useful before shutdown without cef_shutdown()). Optional |callback| on UI thread.
	close_all_connections: proc "c" (self: ^Request_context, callback: ^Completion_callback),

	// Resolve |origin| to IP addresses. |callback| executes on UI thread after completion.
	resolve_host: proc "c" (self: ^Request_context, origin: ^cef_string, callback: ^Resolve_callback),

	// MediaRouter for this context. If |callback| non-nil, executed asynchronously on UI thread after init.
	get_media_router: proc "c" (self: ^Request_context, callback: ^Completion_callback) -> ^Media_router,
	...
	set_chrome_color_scheme: proc "c" (self: ^Request_context, variant: Color_variant, user_color: cef_color),

	// Get current Chrome color scheme mode (SYSTEM/LIGHT/DARK). UI thread only.
	get_chrome_color_scheme_mode: proc "c" (self: ^Request_context) -> Color_variant,

	// Get current Chrome color scheme color (or 0 for default). UI thread only.
	get_chrome_color_scheme_color: proc "c" (self: ^Request_context) -> cef_color,

	// Get current Chrome color scheme variant. UI thread only.
	get_chrome_color_scheme_variant: proc "c" (self: ^Request_context) -> Color_variant,
	...
	add_setting_observer: proc "c" (self: ^Request_context, observer: ^cef_setting_observer) -> ^Registration,
}
