...
// cef_string_multimap is defined in cef_string_capi.odin
...
server :: struct {
    ...
    /// server_handler::on_server_created documentation for a description of
    /// server lifespan.
    ///
    ...
    /// connections. See server_handler::on_server_created documentation for a
    ...
    ///
    /// Close the connection identified by |connection_id|. See send_http_response
    /// documentation for intended usage.
    ///
...
///
/// The server will continue running until server::shutdown() is called, after
/// which time server_handler::on_server_destroyed() will be called on the
...
///
/// This function may be called on any thread in the browser process. The
/// server_handler functions will be called on the dedicated server thread.
///
server_create :: proc "c" (address: ^cef_string, port: u16, backlog: c.int, handler: ^server_handler)

///
/// Implement this structure to handle server events. The functions of this
/// structure will be called on the dedicated server thread.
///
...
server_handler :: struct {
    ...
    /// then server::is_running will return true (1). The server will
    ...
    /// and |connection_id| via the on_web_socket_request callback. See
