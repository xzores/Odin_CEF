...
task_manager :: struct {
    ...
    ///
    get_tasks_count: proc "c" (self: ^task_manager) -> c.size_t,
    ...
    ///
    get_task_ids_list: proc "c" (self: ^task_manager, task_idsCount: ^c.size_t, task_ids: ^i64) -> b32,
    ...
    ///
    kill_task: proc "c" (self: ^task_manager, task_id: i64) -> b32,

    ///
    /// Returns the task ID associated with the main task for |browser_id| (value
    /// from browser::get_identifier). Returns -1 if |browser_id| is invalid,
    /// does not currently have an associated task, or the function was called
    /// from the incorrect thread.
    ///
    get_task_id_for_browser_id: proc "c" (self: ^task_manager, browser_id: c.int) -> i64,
}
...
///
task_manager_get :: proc "c" () -> ^task_manager 